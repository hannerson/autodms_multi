...
class dispatcher(object):
	...
	def get_data2(self,type,conn,cur,table,limit,numbyone):
		...
		if table == "AlbumSrc":
			...
			sql = '''select %s from %s where m_status in (%s) and m_status_art=%s and m_artists!="" and m_name!="" and m_album_id=7392290 and file_sig1>0 order by priority desc limit %s''' % (sql_fields.rstrip(","), table, "7,9", g_status["artist_ok"],limit)
		self.logging.info(sql)
	...
	def dispatch(self,type,table,limit,numbyone,tasklimit,pool_Src,pool_Run,pool_Res):
		...
		if type == "Album":
			...
			count_sql = 'select count(*) from %s where m_status in (7,9) and m_status_art=3 and m_album_id=7392290' % table
