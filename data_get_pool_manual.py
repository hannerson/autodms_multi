...
def get_new_album_track(last_get_time,conn,cur,limit):
	...
	sql = '''select * from CenterApi where track_id in (218751658,218751662,218751659,218751660,218751661,218751667,218751664,218751666,218751677,218751675,218751665,218751668,218751672,218751676,218751683,218751682,218751681,218751678,218751680,218751670,218751671,218751684,218751674,218751673,218751679,218751663,218751669)''' % ()
	cnt = cur.execute(sql)
	if cnt > 0:
		rets = cur.fetchall()
		for ret in rets:
...
def update_MusicSrc(config,mid,info,connData,curData,connSrc,curSrc):
	...
	if cnt > 0:
		connSrc.commit()
	sql = '''update `MusicSrc` set retry_count=0,sig_count=0,m_status=2 where mid=%s''' % (mid)
	cnt = curSrc.execute(sql)
	if cnt > 0:
		connSrc.commit()
