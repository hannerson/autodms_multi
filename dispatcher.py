...
class dispatcher(object):
	...
	def get_data(self,type,conn,cur,table,limit,numbyone):
		...
		if table == "AlbumSrc":
			...
			sql = '''select %s from %s where m_status in (%s) and m_status_art=%s and m_artists!="" and m_name!="" and m_album_id > 0 and file_sig1>0 order by priority desc limit %s''' % (sql_fields.rstrip(","), table, sql_status.rstrip(","), g_status["artist_ok"],limit)
	...
	def dispatch(self,type,table,limit,numbyone,tasklimit,pool_Src,pool_Run,pool_Res):
		...
		if type == "Album":
			...
			count_sql = 'select count(*) from %s where m_status in (2,6) and m_status_art=3' % table
